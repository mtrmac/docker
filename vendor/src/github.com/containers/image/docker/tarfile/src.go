...
// It is acceptable to set path to "", in which case any operation will fail.
...
func (s *Source) openTarComponent(componentPath string) (io.ReadCloser, error) {
	if s.tarPath == "" {
		return nil, errors.New("Internal error: reading tar data without a tar file is not supported")
	}
...
func (s *Source) GetOriginalManifest(instanceDigest *digest.Digest) ([]byte, string, error) {
